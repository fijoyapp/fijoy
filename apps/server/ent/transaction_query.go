...
func (tq *TransactionQuery) QueryTransactionEntries() *TransactionEntryQuery {
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withTransactionEntries: tq.withTransactionEntries.Clone(),
...
func (tq *TransactionQuery) WithTransactionEntries(opts ...func(*TransactionEntryQuery)) *TransactionQuery {
	query := (&TransactionEntryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTransactionEntries = query
	return tq
}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withTransactionEntries; query != nil {
		if err := tq.loadTransactionEntries(ctx, query, nodes,
	...
	for name, query := range tq.withNamedTransactionEntries {
		if err := tq.loadTransactionEntries(ctx, query, nodes,
...
func (tq *TransactionQuery) loadTransactionEntries(ctx context.Context, query *TransactionEntryQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *TransactionEntry)) error {
...
func (tq *TransactionQuery) WithNamedTransactionEntries(name string, opts ...func(*TransactionEntryQuery)) *TransactionQuery {
	query := (&TransactionEntryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedTransactionEntries == nil {
		tq.withNamedTransactionEntries = make(map[string]*TransactionEntryQuery)
	}
	tq.withNamedTransactionEntries[name] = query
	return tq
}
